...
async function downloadSessionData() {
    ...
    const sessdata = config.SESSION_ID.split("KANGO~")[1];
...
async function start() {
    try {
        ...
        console.log(`🤖 XENOCRYPT using WA v${version.join('.')}, isLatest: ${isLatest}`);
        
        const Matrix = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: useQR,
            browser: ["xenocrypt", "safari", "3.3"],
            auth: state,
    ...
    try {
        ...
        if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_SEEN) {
            ...
            if (config.AUTO_STATUS_REPLY) {
                const customMessage = config.STATUS_READ_MSG || '✅ Auto Status Seen Bot By XΞNØCRYPT';
